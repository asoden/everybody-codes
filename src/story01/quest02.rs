...
impl Node<'_> {
    fn new(rank: u64, label: &str) -> Node<'_> {
        Node {
            rank,
            label,
            left: None,
...
pub fn part2(notes: &str) -> String {
    build(notes, |node1, node2| {
        swap(&mut node1.rank, &mut node2.rank);
        swap(&mut node1.label, &mut node2.label);
    })
}
...
fn build(notes: &str, swapper:  for<'a> fn(node1: &mut Node<'a>, node2: &mut Node<'a>)) -> String {
    ...
    for line in notes.lines() {
        if line.starts_with("ADD") {
            ...
            tree.push(Node::new(tokens[1].parse().unwrap(), tokens[2]));
            tree.push(Node::new(tokens[4].parse().unwrap(), tokens[5]));
...
fn bfs(nodes: &Tree<'_>, from: usize) -> String {
    ...
    while let Some((index, depth)) = todo.pop_front() {
        ...
        messages[depth].push_str(nodes[index].label);
